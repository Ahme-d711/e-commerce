...
import { useDeleteProduct, useGetAllProduct } from '../server/productService';
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-[var(--color-foreground)]">Dashboard / Products</h1>
        <Link to="/products/create-product" className="btn btn-sm bg-[var(--color-primary)] text-[var(--color-primary-foreground)] rounded-[var(--radius-md)] px-3 py-2">
          <PlusCircle className="w-4 h-4 mr-2 inline" /> New Product
        </Link>
      </div>
              ...
              {items.map((p) => (
                <motion.tr key={p._id} variants={rowVariants} className="border-t border-[var(--color-border)]">
                  <td className="px-4 py-3 flex gap-3 items-center text-[var(--color-foreground)]">
                    <img src={p.imageUrl || 'https://cdn-icons-png.freepik.com/256/15980/15980207.png?semt=ais_white_label'} alt={p.name} className="w-16 h-16 rounded-full object-cover" />
                    <span className="line-clamp-2 leading-snug">{p.name}</span>
                  </td>
                  <td className="px-4 py-3 text-[var(--color-muted-foreground)]">{p.category}</td>
                  <td className="px-4 py-3 text-[var(--color-foreground)]">${p.price.toFixed(2)}</td>
                  <td className="px-4 py-3 text-[var(--color-muted-foreground)]">{dayjs(p.updatedAt).format("DD/MM/YYYY")}</td>
                  <td className="px-4 py-3 text-[var(--color-foreground)]">{p.countInStock}</td>
